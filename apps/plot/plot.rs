...
fn draw_line_plot(
    (x_axis, y_axis): (Vec<f32>, Vec<(&str, Vec<f32>)>),
    path: String,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (json, svg_path) in missing_svg {
        ...
        let (x_axis, y_axis): (Vec<f32>, Vec<(&str, Vec<f32>)>) = serde_json::from_str(&json)?;
        draw_line_plot((x_axis, y_axis), svg_path.display().to_string())?;
    }

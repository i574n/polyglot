...
if (!$fast) {
    { . ../../workspace/target/release/spiral_builder$(_exe) dib --path Supervisor.dib --retries 3 } | Invoke-Block
}
...
if (!$fast) {
    { . ../../workspace/target/release/spiral_builder$(_exe) dib --path Eval.dib --retries 3 } | Invoke-Block
}

                    ...
                    | line when
                        ...
                        let m =
                            System.Text.RegularExpressions.Regex.Match (
                                line,
                                @"^(?:and +)?(inl|let) +([~\(\w][\w\d']*(?:| *[~\w][\w\d']*\)|, *[~\w][\w\d']*)) +[:=](?! +function)"
                            )
                            ...
                            ||> Async.fold (fun acc buildCodeResult -> async {
                                ...
                                | backend, (_, (outputPath, Some code), spiralErrors) ->
                                    ...
                                    | (Error ex, errors), _ | _, (Error ex, errors) ->
                                        return
                                            Error (Exception $"Eval.eval / processSpiralOutput / Exception / buildCodeResult: %A{buildCodeResult} / ex: {ex |> SpiralSm.format_exception}"),

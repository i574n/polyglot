...
module Eval =
    ...
    let inline eval
        ...
        let traceLevel =
            ...
            if traceToggled |> not
            then oldLevel |> from_trace_level
            else
                let newLevel = Verbose
                ...
                use _ = (new_disposable (fun () ->
                    trace Verbose
                        (fun () -> $"Eval.eval / disposable")
                        (fun () -> $"oldLevel: {oldLevel} / newLevel: {newLevel} / get_trace_level(): {get_trace_level ()} / {_locals ()}")
        ...
        then
            ...
            let packages =
                if args |> SpiralSm.contains "--package " |> not
                then [||]
                else
                    args
            ...
            let timeout =
                if args |> SpiralSm.contains "--timeout " |> not
                then None
                else
                    args
            ...
            let getArg def value command =
                let command_value =
                    ...
                    | None -> command
                trace Verbose (fun () -> $"Eval.eval / getArg / command_value: {command_value} / value: %A{value} / command: {command} / args: {args}") _locals
                if args |> SpiralSm.contains command_value
                then true
                elif args |> SpiralSm.contains $"{command} false"
                then false
                else def
            ...
            let isTestStatic = "--test" |> getArg false ("static" |> Some)
            let isTest = isTestStatic || "--test" |> getArg false None
            let isCache = "--cache" |> getArg false None

...
def closure1(unit_var: None, unit_var_1: None) -> str:
    ...
    raise Exception(("platform.is_windows / target: " + str(US1(4, US0(0)))) + "")
    _v0 = x
    def _arrow113(__unit: None=None, unit_var: Any=unit_var, unit_var_1: Any=unit_var_1) -> bool:
        raise Exception("base.run_target / _v0=None")

    if _arrow113() if (_v0 is None) else _v0:
        return ".exe"
...
def _arrow114(__unit: None=None) -> bool:
    return closure0(None, None)


v0: Callable[[], bool] = _arrow114

def is_windows(__unit: None=None) -> bool:
    return v0(None)


def _arrow115(__unit: None=None) -> str:
    return closure1(None, None)


v1: Callable[[], str] = _arrow115

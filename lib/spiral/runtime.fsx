...
and closure29 () (v0 : string) : Result<(string []), string> =
    ...
    | US14_0(v2) -> (* Ok *)
        ...
        v3
let v68 : (US0 -> struct (Mut0 * Mut1 * Mut2 * Mut3 * int64 option)) = closure0()
let v69 : US0 = US0_0
if State.trace_state.IsNone then State.trace_state <- v68 v69 |> Some
let v75 : (unit -> unit) = closure2()
let current_process_kill () = v75 ()
let v76 : (string -> Async<struct (int32 * string)>) = closure9()
let execute_async x = v76 x
let v77 : (struct (string * System.Threading.CancellationToken option * (struct (string * string) []) * (struct (int32 * string * bool) -> Async<unit>) option * (std_sync_Arc<std_sync_Mutex<std_process_ChildStdin>> -> unit) option * bool * string option) -> Async<struct (int32 * string)>) = closure27()
let execute_with_options_async x = v77 x
let v78 : ((Heap0 -> Heap0) -> struct (string * System.Threading.CancellationToken option * (struct (string * string) []) * (struct (int32 * string * bool) -> Async<unit>) option * (std_sync_Arc<std_sync_Mutex<std_process_ChildStdin>> -> unit) option * bool * string option)) = closure28()

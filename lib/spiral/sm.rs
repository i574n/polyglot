pub mod Sm {
    ...
    pub fn closure15(v0: string, v1: string, v2: string) -> string {
        ...
        let v7: string = Sm::method1(v2);
        let v9: Result<regex::Regex, regex::Error> = regex::Regex::new(&v0);
        let v11: regex::Regex = v9.unwrap();
        let v13: std::borrow::Cow<str> = v11.replace_all(&v7, &*v3);
    ...
    pub fn closure22(v0: Array<string>, v1: string) -> Array<string> {
        ...
        let v7: LrcPtr<dyn IEnumerable_1<string>> = ofArray(v0);
        {
            let x: Array<string> = split_1(v1, join(Sm::method3(), toArray(v7)), -1_i32, 0_i32);
            _v4.set(Some(x))
        }
        match &_v4.get().clone() {

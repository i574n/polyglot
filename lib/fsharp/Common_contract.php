...
function trace($level, $fn, $getLocals) {
    return traceRaw($level, function ($unitVar) use ($fn, $getLocals, $level, $traceCount) {     return \lib\SpiralSm_trim_end([ ' ', '/' ])(\lib\SpiralSm_trim_start([  ])(\String\toText(\String\interpolate('%P() #%P() [%A%P()] %s%P() / %s%P()', [ '', $GLOBALS['traceCount'], $level, $fn(NULL), $getLocals(NULL) ]))));
 });
}
